...
public class Bba_Teacher_Detail extends AppCompatActivity {
    Toolbar tl ;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_bba__teacher__detail);
        cn = this;
        mView = new CatLoadingView();
        mView.show(getSupportFragmentManager(),"Loading.........");
        Toolbar();
        bbafaclist = (ListView) findViewById(R.id.bbafaclist);
        req = Volley.newRequestQueue(this);
        preferences = getSharedPreferences("JsonData",cn.MODE_PRIVATE);
        if(!haveNetworkConnection()){
            String res = preferences.getString("bbafaculty","null");
            if(!res.equals("null")) {
                Gson gson = new Gson();
                teacherJson = gson.fromJson(res, TeacherJson.class);
                bbafaclist.setAdapter(new TList(cn, teacherJson.getBba()));
                if(mView!=null){
                    mView.dismiss();
                }
            }else {
                Snackbar.make(findViewById(R.id.bbafactol),"No Internet Connection",Snackbar.LENGTH_LONG)
                        .show();
            }
        }else{
            netWork();
    ...
    void Toolbar (){
        tl= (Toolbar) findViewById(R.id.bbafactol);
    ...
    void netWork(){
        JsonObjectRequest jsr = new JsonObjectRequest(Request.Method.GET, link, new Response.Listener<JSONObject>() {
            ...
            public void onResponse(JSONObject response) {
                ...
                editor.putString("bbafaculty",result);
                editor.commit();
                Gson gson = new Gson();
    ...
    class TList extends BaseAdapter {
        private Context cn;
        List<TeacherJson.BbaBean> bba;
        public TList(Context cn, List<TeacherJson.BbaBean> bba) {
            this.cn = cn;
            this.bba=bba;
        }
        @Override
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            TeacherJson.BbaBean c = bba.get(position);
